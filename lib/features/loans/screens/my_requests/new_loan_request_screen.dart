...
class _NewLoanRequestScreenState extends State<NewLoanRequestScreen> {
  ...
  void _onAmountChanged() {
    ...
    final installment =
        double.tryParse(_installmentValueController.text) ?? 0.0;
    ...
    if (focusedHashCode != _totalValueController.hashCode &&
      ...
      _installmentValueController.addListener(_onAmountChanged);
    } else if (focusedHashCode != _installmentCountController.hashCode &&
        total > 0 &&
        installment > 0) {
      _installmentCountController.removeListener(_onAmountChanged);
      _installmentCountController.text = (total / installment)
          .round()
          .toString();
      _installmentCountController.addListener(_onAmountChanged);
    }
  ...
  Future<void> _saveForm() async {
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.loanRequestSentSuccessfully),
          ...
          child: Column(
            ...
            children: [
              ...
              _buildTextField(
                ...
                l10n.installmentsCount,
                Icons.format_list_numbered,
                isNumber: true,
              ),
              const SizedBox(height: 16),
              _buildTextField(
                _installmentValueController,
                l10n.installmentValue,
                Icons.payment,
                isNumber: true,
              ),
              const SizedBox(height: 16),
      ...
      items: loanTypes
          .map(
            (type) => DropdownMenuItem<int>(
              value: type.loanTypeCode,
              child: Text(
                isArabic
                    ? (type.nameA ?? '')
                    : (type.nameE ?? type.nameA ?? ''),
              ),
            ),
          )
          .toList(),
  ...
  Widget _buildDatePicker() {
    ...
    return TextFormField(
      readOnly: true,
      decoration: InputDecoration(
        labelText: l10n.repaymentStartDate,
      ...
      controller: TextEditingController(
        text: _startDate == null
            ? l10n.selectDate
            : DateFormat.yMd(locale.toLanguageTag()).format(_startDate!),
      ),
