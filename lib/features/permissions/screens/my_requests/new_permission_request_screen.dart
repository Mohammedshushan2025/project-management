...
class NewPermissionRequestScreen extends StatefulWidget {
  ...
  @override
  State<NewPermissionRequestScreen> createState() =>
      _NewPermissionRequestScreenState();
}

class _NewPermissionRequestScreenState
    extends State<NewPermissionRequestScreen> {
  final _formKey = GlobalKey<FormState>();
  ...
  Future<void> _selectEndDate() async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate:
          _permissionDate ?? DateTime.now().subtract(const Duration(days: 30)),
      lastDate: DateTime.now().add(const Duration(days: 365)),
    );
  ...
  void _submitForm() async {
    ...
    final int targetCompEmpCode = _isForWorker
        ...
        : user.compEmpCode;
    final String targetUserName = _isForWorker
        ? _selectedWorker!.empName
        : user.usersName;
    final int insertingUserCode = user.usersCode;

    // التحقق من الوقت فقط في الحالات 1 و 4
    bool isTimeRequired =
        _selectedPermissionType == 1 || _selectedPermissionType == 4;
    ...
    if (isTimeRequired) {
      ...
      if (toDateTime.isBefore(fromDateTime) ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.timeValidationError),
            backgroundColor: Colors.red,
          ),
        );
        return;
    ...
    if (_selectedPermissionType == 5) {
      if (_permissionEndDate!.isBefore(_permissionDate!)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.endDateAfterStartDateError),
            backgroundColor: Colors.red,
          ),
        );
        return;
    ...
    String permissionTypeName = _getPermissionTypeName(
      _selectedPermissionType!,
    );
    final String notes =
        "طلب $permissionTypeName الموظف $targetUserName عن يوم $prmDateStr$timeStr";
    ...
    final bool success = await hrProvider.createPermissionRequest(
      ...
      reasonType: _selectedReasonType!,
      prmDate: _permissionDate!,
      prmToDate: _selectedPermissionType == 5
          ? _permissionEndDate
          : null, // إرسال التاريخ الثاني فقط في حالة الانتداب
      fromTime: fromDateTime,
      toTime: toDateTime,
      permReasons: _reasonsController.text.isEmpty
          ? permissionTypeName
          : _reasonsController.text,
      notes: notes,
    );
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.permissionRequestSentSuccessfully),
                  ...
                  children: [
                    ...
                    _buildDropdown(
                      ...
                      validator: (value) => value == null
                          ? l10n.selectPermissionTypeValidation
                          : null,
                    ),
                    const SizedBox(height: 16),
                    ...
                    _buildDropdown(
                      ...
                      validator: (value) => value == null
                          ? l10n.selectReasonTypeValidation
                          : null,
                    ),
                    const SizedBox(height: 16),
                    ...
                    _buildDatePicker(
                      ...
                      validator: (value) => _permissionDate == null
                          ? l10n.selectPermissionDateValidation
                          : null,
                    ),
                    ...
                    if (showEndDate) ...[
                      ...
                      _buildDatePicker(
                        label:
                            l10n.permissionEndDateLabel ??
                            "End Date", // تاريخ الانتهاء
                        date: _permissionEndDate,
                        onPressed: _selectEndDate,
                        validator: (value) => _permissionEndDate == null
                            ? (l10n.selectPermissionEndDateValidation ??
                                  "Select end date")
                            : null,
                      ),
                    ],
    ...
    return TextFormField(
      ...
      validator: (value) => time == null
          ? (label == l10n.fromTimeLabel
                ? l10n.selectFromTimeValidation
                : l10n.selectToTimeValidation)
          : null,
    );
  }
