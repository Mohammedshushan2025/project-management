// lib/features/cancel_salary_confirmation/screens/cancel_salary_confirmation_request_details_screen.dart
...
import '../models/cancel_salary_confirmation_auth_model.dart';
...
class CancelSalaryConfirmationRequestDetailsScreen extends StatefulWidget {
  static const String routeName = '/cancel-salary-confirmation-request-details';
  const CancelSalaryConfirmationRequestDetailsScreen({super.key});

  @override
  State<CancelSalaryConfirmationRequestDetailsScreen> createState() =>
      _CancelSalaryConfirmationRequestDetailsScreenState();
}

class _CancelSalaryConfirmationRequestDetailsScreenState
    extends State<CancelSalaryConfirmationRequestDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (hrProvider.selectedCancelSalaryConfirmationRequest != null) {
        hrProvider.loadCancelSalaryConfirmationAuthDetails();
      }
    });
  }
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (builderContext, setDialogState) {
            ...
            return AlertDialog(
              title: Text(l10n.cancelSalaryConfirmationActionDialogTitle),
              content: Column(
  ...
  Future<void> _submitAction(
    ...
    final success = await hrProvider.submitAction(
      ...
      authFlag: authFlag,
      authTableName: 'SS_FIXED_SAL',
      authPk1: request.empCode.toString(),
      authPk2: request.serial.toString(),
      authChain: hrProvider.cancelSalaryConfirmationAuthDetails?.items ?? [],
      systemNumber: 63,
      fileSerial: 13,
    );
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<HrProvider>(
      builder: (context, hrProvider, child) {
        final request = hrProvider.selectedCancelSalaryConfirmationRequest;
        if (request == null) {
          return Scaffold(
            appBar: AppBar(),
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(l10n.cancelSalaryConfirmationRequestDetails),
  ...
  Widget _buildHeaderCard(CancelSalaryConfirmationRequestItem request) {
    final l10n = AppLocalizations.of(context)!;
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(
              ...
              _formatDate(request.trnsDate, "yyyy-MM-dd"),
            ),
            if (request.notes != null && request.notes!.isNotEmpty)
              _buildDetailRow(l10n.notesLabel, request.notes!),
            const SizedBox(height: 16),
            SizedBox(
  ...
  Widget _buildAuthDetailsTab(HrProvider provider) {
    ...
    if (provider.isLoading &&
        provider.cancelSalaryConfirmationAuthDetails == null) {
      return const Center(
        heightFactor: 5,
        child: SpinKitFadingCircle(color: AppColors.primaryColor),
      );
    }
    final authItems = provider.cancelSalaryConfirmationAuthDetails?.items;
  ...
  Widget _buildAuthTimeline(List<CancelSalaryConfirmationAuthItem> authItems) {
    final l10n = AppLocalizations.of(context)!;
    final localeProvider = Provider.of<LocaleProvider>(context, listen: false);
    final isArabic = localeProvider.locale.languageCode == 'ar';
