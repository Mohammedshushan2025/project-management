...
class SectionWithSliderWidget extends StatefulWidget {
  ...
  @override
  _SectionWithSliderWidgetState createState() =>
      _SectionWithSliderWidgetState();
}

class _SectionWithSliderWidgetState extends State<SectionWithSliderWidget>
    with TickerProviderStateMixin {
  late PageController _pageController;
  int _currentPage = 0;
  Timer? _autoSlideTimer;
  late AnimationController
  _sliderItemPressAnimationController; // تم تغيير الاسم للوضوح
  ...
  void initState() {
    super.initState();
    print(
      "[SectionWithSliderWidget - ${widget.sectionTitle}] initState called. Items: ${widget.menuItems.length}",
    );
    ...
    double viewportFractionValue = widget.menuItems.length > 1
        ? 0.85
        : 1.0; // إذا عنصر واحد، خليه يملأ العرض
    ...
    _pageController.addListener(() {
      if (_pageController.page != null) {
        ...
        if (newPage >= 0 &&
            newPage < widget.menuItems.length &&
            _currentPage != newPage &&
            mounted) {
          setState(() {
            _currentPage = newPage;
          });
        } else if (widget.menuItems.isEmpty && _currentPage != 0 && mounted) {
  ...
  void _startAutoSlide() {
    ...
    _autoSlideTimer = Timer.periodic(const Duration(seconds: 5), (Timer timer) {
      ...
      if (mounted &&
        ...
        // التأكد أن هناك أكثر من عنصر للسلايد
  ...
  Widget _buildMainServicesSlider() {
    if (widget.menuItems.isEmpty) {
      print(
        "[SectionWithSliderWidget - ${widget.sectionTitle}] No items to build slider.",
      );
    ...
    print(
      "[SectionWithSliderWidget - ${widget.sectionTitle}] Building slider with ${widget.menuItems.length} items. Current page: $_currentPage",
    );
      ...
      child: PageView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (_pageController.position.haveDimensions) {
            ...
            // حالة قبل أن يكون للـ controller أبعاد
            scale = 0.88;
          }
          // التأكد أن index ضمن الحدود قبل محاولة الوصول للعنصر
          if (index >= widget.menuItems.length || index < 0)
  ...
  Widget _buildSliderItem(HomeMenuItemData item, int index, double scale) {
    ...
    bool isActive =
        (index == _currentPage &&
        _currentPage < widget.menuItems.length &&
        _currentPage >= 0);
    print(
      "[SectionWithSliderWidget - ${widget.sectionTitle}] Building slider item $index. Title: ${item.title}. IsActive: $isActive. Scale: $scale",
    );
    ...
    return Transform.scale(
      ...
      child: GestureDetector(
        onTapDown: (_) {
          if (isActive && mounted)
            _sliderItemPressAnimationController.forward();
        },
        onTapUp: (_) {
          if (mounted) {
        ...
        child: AnimatedBuilder(
          ...
          builder: (context, child) {
            double pressScale = isActive
                ? (1 - (_sliderItemPressAnimationController.value * 0.03))
                : 1.0;
                            ...
                            if (item.notificationCount != null &&
                              ...
                              Container(
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    color:
                                        item.color, // لون الإشعار بلون الكارت
                                    fontSize: 10,
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
  ...
  Widget build(BuildContext context) {
    print(
      "[SectionWithSliderWidget - ${widget.sectionTitle}] build called. Items: ${widget.menuItems.length}",
    );
    if (widget.menuItems.isEmpty && !mounted) {
      // إضافة تحقق من mounted هنا أيضاً
      // إذا لم يكن هناك عناصر والويدجت لم يعد mounted (نادر ولكن للاحتياط)
      return const SizedBox.shrink();
    }

    return Column(
      // لا نحتاج لـ SlideTransition و FadeTransition هنا إذا كان الأنيميشن داخل HomeScreen
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildCategoryTitle(
          widget.sectionTitle,
          widget.sectionIcon,
          widget.sectionNotificationCount,
        ),
        ...
        if (widget.menuItems.isNotEmpty)
          _buildPageIndicator(), // فقط إذا كانت هناك عناصر
        if (widget.menuItems.isEmpty)
          const SizedBox(height: 25), // للحفاظ على مسافة إذا كان فارغًا
