// lib/features/salary_confirmation/screens/salary_confirmation_request_details_screen.dart
...
import '../models/salary_confirmation_auth_model.dart';
import '../models/salary_confirmation_request_model.dart';
import '../../../core/providers/hr_provider.dart';
import '../../../core/providers/auth_provider.dart';
import '../../../core/providers/locale_provider.dart';
import '../../../core/utils/app_colors.dart';

class SalaryConfirmationRequestDetailsScreen extends StatefulWidget {
  static const String routeName = '/salary-confirmation-request-details';
  const SalaryConfirmationRequestDetailsScreen({super.key});

  @override
  State<SalaryConfirmationRequestDetailsScreen> createState() =>
      _SalaryConfirmationRequestDetailsScreenState();
}

class _SalaryConfirmationRequestDetailsScreenState
    extends State<SalaryConfirmationRequestDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (hrProvider.selectedSalaryConfirmationRequest != null) {
        hrProvider.loadSalaryConfirmationAuthDetails();
      }
    });
  }
  ...
  void _showActionDialog(
    ...
    SalaryConfirmationRequestItem request,
  ) {
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (builderContext, setDialogState) {
            ...
            return AlertDialog(
              title: Text(l10n.salaryConfirmationActionDialogTitle),
              content: Column(
  ...
  Future<void> _submitAction(
    ...
    final success = await hrProvider.submitAction(
      ...
      authChain: hrProvider.salaryConfirmationAuthDetails?.items ?? [],
      systemNumber: 63,
      fileSerial: 12,
    );
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<HrProvider>(
      builder: (context, hrProvider, child) {
        final request = hrProvider.selectedSalaryConfirmationRequest;
        if (request == null) {
          return Scaffold(
            appBar: AppBar(),
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(l10n.salaryConfirmationRequestDetails),
  ...
  Widget _buildHeaderCard(SalaryConfirmationRequestItem request) {
    final l10n = AppLocalizations.of(context)!;
  ...
  Widget _buildAuthDetailsTab(HrProvider provider) {
    final l10n = AppLocalizations.of(context)!;
    if (provider.isLoading && provider.salaryConfirmationAuthDetails == null) {
      return const Center(
        heightFactor: 5,
        child: SpinKitFadingCircle(color: AppColors.primaryColor),
      );
    }
    final authItems = provider.salaryConfirmationAuthDetails?.items;
  ...
  Widget _buildAuthTimeline(List<SalaryConfirmationAuthItem> authItems) {
    final l10n = AppLocalizations.of(context)!;
    final localeProvider = Provider.of<LocaleProvider>(context, listen: false);
    final isArabic = localeProvider.locale.languageCode == 'ar';
