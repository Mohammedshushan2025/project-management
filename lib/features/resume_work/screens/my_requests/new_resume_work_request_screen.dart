...
class NewResumeWorkRequestScreen extends StatefulWidget {
  ...
  @override
  State<NewResumeWorkRequestScreen> createState() =>
      _NewResumeWorkRequestScreenState();
}

class _NewResumeWorkRequestScreenState
    extends State<NewResumeWorkRequestScreen> {
  ...
  void _submitForm() async {
    ...
    final String targetUserName = _isForWorker
        ...
        : authProvider.currentUser!.usersName; // للاستخدام في الملاحظات
    ...
    if (_actualResumeDate!.isBefore(_startDate!)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l10n.resumeDateAfterStartDateError),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    final String resumeDateStr = DateFormat.yMd(
      locale,
    ).format(_actualResumeDate!);
    ...
    final bool success = await hrProvider.createResumeWorkRequest(
      userCode:
          insertingUserCode, // تمرير المستخدم الحالي كـ userCode لجلب السيريال
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.resumeWorkRequestSentSuccessfully),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDatePicker(
                      ...
                      validator: (value) =>
                          _startDate == null ? l10n.selectStartDate : null,
                    ),
                    ...
                    _buildDatePicker(
                      ...
                      validator: (value) =>
                          _endDate == null ? l10n.selectEndDate : null,
                    ),
                    const SizedBox(height: 16),
                    _buildDatePicker(
                      ...
                      onPressed: () => _selectDate(
                        context,
                        onDateSelected: (date) =>
                            setState(() => _actualResumeDate = date),
                      ),
                      validator: (value) => _actualResumeDate == null
                          ? l10n.selectResumeDate
                          : null,
                    ),
                    const SizedBox(height: 16),
                    _buildTextField(
                      _lateReasonController,
                      l10n.delayReason,
