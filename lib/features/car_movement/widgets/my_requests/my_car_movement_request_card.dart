// lib/features/car_movement/widgets/my_requests/my_car_movement_request_card.dart
...
class MyCarMovementRequestCard extends StatelessWidget {
  final MyCarMovementRequestItem request;
  final VoidCallback onTap;

  const MyCarMovementRequestCard({
    super.key,
  ...
  String _formatTime(String? timeString, String locale) {
    ...
    try {
      // الـ API بيرجع تاريخ + وقت، احنا محتاجين الوقت بس
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // ---== تم التعديل: استخدام الليبلز الصحيحة ==---
              _buildInfoRow(
                Icons.calendar_today_outlined,
                l10n.carMovementDateLabel,
                _formatDate(request.prmDate, locale),
              ),
              _buildInfoRow(
                Icons.access_time_outlined,
                l10n.carMovementTimeLabel,
                '${_formatTime(request.fromTime, locale)} - ${_formatTime(request.toTime, locale)}',
              ),

              // ---== نهاية التعديل ==---
              if (request.permReasons != null &&
                  request.permReasons!.isNotEmpty)
                _buildInfoRow(
