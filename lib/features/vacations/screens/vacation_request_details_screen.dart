// lib/features/vacations/screens/vacation_request_details_screen.dart
...
class VacationRequestDetailsScreen extends StatefulWidget {
  static const String routeName = '/vacation-request-details';
  const VacationRequestDetailsScreen({super.key});

  @override
  State<VacationRequestDetailsScreen> createState() =>
      _VacationRequestDetailsScreenState();
}

class _VacationRequestDetailsScreenState
    extends State<VacationRequestDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (hrProvider.selectedVacationRequest != null) {
        hrProvider.loadVacationAuthDetails();
      }
    });
  }

  void _showActionDialog(BuildContext context, VacationRequestItem request) {
  ...
  Future<void> _submitAction(
    ...
    final success = await hrProvider.submitAction(
      ...
      authFlag: authFlag,
      authTableName: 'PY_ORDER_VCNC_H',
      authPk1: request.empCode.toString(),
      authPk2: request.serialPyv.toString(),
      authChain: hrProvider.vacationAuthDetails?.items ?? [],
      systemNumber: 61,
      fileSerial: 9,
    );
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<HrProvider>(
      builder: (context, hrProvider, child) {
        final request = hrProvider.selectedVacationRequest;
        if (request == null) {
          return Scaffold(
            appBar: AppBar(),
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(l10n.vacationRequestDetailsTitle),
  ...
  Widget _buildHeaderCard(VacationRequestItem request) {
    final l10n = AppLocalizations.of(context)!;
    ...
    final vacationType = isArabic
        ? request.vacationTypeString
        : (request.vacationTypeStringE ?? request.vacationTypeString);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(
              l10n.vacationTypeLabel,
              vacationType ?? l10n.notSpecified,
            ),
            _buildDetailRow(
              l10n.startDateLabel,
              _formatDate(request.startDt, "yyyy-MM-dd"),
            ),
            _buildDetailRow(
              l10n.endDateLabel,
              _formatDate(request.endDt, "yyyy-MM-dd"),
            ),
            _buildDetailRow(
              l10n.durationLabel,
  ...
  Widget _buildAuthDetailsTab(HrProvider provider) {
    final l10n = AppLocalizations.of(context)!;
    if (provider.isLoading && provider.vacationAuthDetails == null) {
      return const Center(
        heightFactor: 5,
        child: SpinKitFadingCircle(color: AppColors.primaryColor),
      );
    }
    final authItems = provider.vacationAuthDetails?.items;
  ...
  Widget _buildAuthTimeline(List<VacationAuthItem> authItems) {
    final l10n = AppLocalizations.of(context)!;
    final localeProvider = Provider.of<LocaleProvider>(context, listen: false);
    final isArabic = localeProvider.locale.languageCode == 'ar';
    ...
    return Padding(
      ...
      child: Column(
        children: List.generate(authItems.length, (index) {
          ...
          // ... (logic for colors and icons remains the same)
