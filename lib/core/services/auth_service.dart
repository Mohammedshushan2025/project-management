...
class AuthService {
  Future<User?> login(String usersCode, String password) async {
    try {
      final response = await http.get(
        ...
        }, // هذا Header للطلب، ليس للاستجابة
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedResponse.containsKey('items') &&
          ...
          for (var userData in usersData) {
            if (userData is Map<String, dynamic>) {
              if (userData.containsKey('UsersCode') &&
        ...
        try {
          ...
          print(
            'Response body (UTF-8 decoded): ${utf8.decode(response.bodyBytes)}',
          );
        } catch (e) {
          print('Could not decode response body for error logging: $e');
        }
        throw Exception(
          'Failed to load users from API. Status Code: ${response.statusCode}',
        );
      }
    } catch (e) {
  ...
  Future<ProjectCategoriesCount?> getProjectCategoriesCount({
    required String usersCode,
  }) async {
    try {
      final response = await http.get(
        Uri.parse(
          '${ApiConstants.baseUrl}${ApiConstants.projectCategoriesCountEndpoint}$usersCode',
        ),
        headers: {'Content-Type': 'application/json'},
      );
      ...
      if (response.statusCode == 200) {
        String responseBody = utf8.decode(response.bodyBytes);
        final ProjectCategoriesCount projectCategoriesCount =
            ProjectCategoriesCount.fromJson(json.decode(responseBody));
        return projectCategoriesCount;
      } else {
        throw Exception('Failed to load project categories count data.');
      }
    } catch (e) {
      print('Error getting project categories count: $e');
      throw Exception(
        'An error occurred while fetching project categories count: $e',
      );
    }
  }
